...
# normal
# train model
adult_ranger$train(task_po_rw_wts$output, split$train)

# prediction
adult_ranger_prd <- adult_ranger$predict(task_po_rw_wts$output, split$test)
#adult_ranger_prd$set_threshold(0.5)

# scoring
adult_ranger_prd$score(msout, task_po_rw_wts$output)

# classif.auc fairness.pp_np 
# 0.91857311     0.08353031

# ###
# # resampling
# rr_rw_wts <- resample(task_po_rw_wts$output, adult_ranger, rsmp("cv", folds = 5))
# #rr_rw_wts <- resample(task_po_rw_wts$output, adult_ranger, rsmp("bootstrap", repeats = 20, ratio = 0.8))
# 
